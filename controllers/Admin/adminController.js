...
const downloadSalesReport = async (req, res) => {
    try {
        ...
        if (format === 'pdf') {
            ...
            topProducts.forEach((p, i) => doc.text(`${p.name}: ${p.units} units, ₹${p.revenue.toFixed(2)}`, 70, doc.y + 15));
            doc.moveDown(1);
            doc.text('Top Categories:', 50);
            topCategories.forEach((c, i) => doc.text(`${c.name}: ${c.units} units, ₹${c.revenue.toFixed(2)}`, 70, doc.y + 15));
            ...
            locationData.forEach((loc, i) => doc.text(`${loc._id}: ₹${loc.sales.toFixed(2)} (${loc.orders} orders)`, 70, doc.y + 15));
